...
open class FolioReaderPage: UICollectionViewCell, WKUIDelegate, UIGestureRecognizerDelegate {
    ...
    open func webView(_ webView: WKWebView, shouldStartLoadWith request: URLRequest, navigationType: WKNavigationType) -> Bool {
        guard
    ...
    @objc func refreshPageMode() {
        ...
        if (self.folioReader.nightMode == true) {
            ...
            let frameHeight = webView.frame.height as CGFloat
            let lastPageHeight = frameHeight //CGFloat(frameHeight * webView.pageCount) - CGFloat(contentHeight)
            colorView.frame = CGRect(x: webView.frame.width, y: webView.frame.height - lastPageHeight, width: webView.frame.width, height: lastPageHeight)
        } else {
